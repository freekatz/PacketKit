...
4. 数据包**统计**与**分析**：
   ...
   - 流量分析：流量大小变化曲线图、 流量协议占比饼状图、 网卡信息、 IPv4 及 IPv6 统计、 数据包统计、 流量统计。
...
5. 捕获或读取到的数据包列表如何高效地管理？
   ...
   > 答：这是一个承上启下的过程，为了扩展需要及其它模块的便利，必须对数据包进行高效地管理。因此，我引入了数据包组的概念，每个组都由在线数据包组和离线数据包组构成，顾名思义，在线组保存实时捕获的包，离线组保存从缓冲区或离线文件读取的包。
   >
...
[@Spring Project](https://github.com/spring-projects/spring-boot)
